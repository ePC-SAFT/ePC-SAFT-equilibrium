...
struct HeldStageIIIPhaseEvaluation {
    std::array<double, 2> amounts_mol{};
    double volume_m3 = 0.0;
    MixturePhaseEvaluation provider{};
};

struct HeldStageIIINlpEvaluation {
    ...
    HeldStageIIIPhaseEvaluation liquid{};
    HeldStageIIIPhaseEvaluation vapor{};
};

[[nodiscard]] HeldStageIIINlpEvaluation evaluate_held_stage_iii_nlp(
...
struct HeldStageIIILocalAttempt {
    std::string role;
...
struct HeldStageIIILocalResult {
    bool accepted = false;
    bool solver_converged = false;
    ...
    std::vector<HeldStageIIILocalAttempt> attempt_log;
