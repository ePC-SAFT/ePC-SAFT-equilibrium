...
Held2StageIIResult solve_held2_stage_ii(
    ...
    const double composition_sum_upper = 1.0
        - kHeld2ModifiedLowerScale
            * coordinates.modified_factors[dependent_position];
    ...
    struct Cut {
        ...
        bool local_state_certified = false;
    };
    ...
    std::vector<Cut> cuts;
    cuts.push_back(make_cut(feed, reference.volume, false));
    for (const Held2StageICandidate& candidate : stage_i_candidates) {
        ...
        cuts.push_back(make_cut(
            independent_from_state(state), candidate.volume, false
        ));
    }
    ...
    result.search_strategy =
        "persistent_m_continuation_sobol_direct_l_ipopt_v2";
    ...
    for (int major = 0; major < major_iteration_cap; ++major) {
        ...
        Cut new_m_point;
        bool new_m_member_certified = false;
            ...
            try {
                ...
                if (attempt.cut_eligible) {
                    const auto known = std::find_if(
                        cuts.begin(),
                        cuts.end(),
                        [&state](const Cut& point) {
                            return point.state.modified_fractions
                                    == state.modified_fractions
                                && point.state.volume == state.volume;
                        }
                    );
                    if (known == cuts.end()
                        || !known->local_state_certified) {
                        new_m_point = make_cut(
                            independent, state.volume, true
                        );
                        new_m_member_certified = true;
                    }
                }
                ...
                if (qualified && new_m_member_certified) {
                    break;
                }
            } catch (const std::exception& error) {
        ...
        if (new_m_member_certified) {
            const auto duplicate_cut = std::find_if(
                cuts.begin(),
                cuts.end(),
                [&new_m_point](const Cut& known) {
                    return known.state.modified_fractions
                            == new_m_point.state.modified_fractions
                        && known.state.volume == new_m_point.state.volume;
                }
            );
            if (duplicate_cut == cuts.end()) {
                cuts.push_back(std::move(new_m_point));
            } else {
                duplicate_cut->local_state_certified = true;
            }
            new_cut_added = true;
        }
        result.candidates.clear();
        for (const Cut& point : cuts) {
            if (!point.local_state_certified) {
                continue;
            }
            double point_value = point.state.objective;
            for (std::size_t coordinate = 0;
                 coordinate < dimension;
                 ++coordinate) {
                point_value += multipliers[coordinate]
                    * (feed[coordinate] - point.independent[coordinate]);
            }
            const Held2StageIIStep6Assessment step6 =
                assess_held2_stage_ii_step6(
                    upper_bound,
            ...
            for (const Held2StageIICandidate& known : result.candidates) {
                const double composition_distance =
                    maximum_abs_difference(
                        known.modified_fractions,
                        point.state.modified_fractions
                    );
                const double log_volume_distance = std::abs(
                    known.phase_coordinate - std::log(point.state.volume)
                );
