...
- [ ] Write parameterized failing tests for valid `A <-> B`, gauge reconstruction, and every compiler rejection class: order, charge, fingerprint, rank, nonconservation, reaction dependence, incomplete `lnK`, reference binding, and non-neutral feed.
- [ ] Run `pytest -q tests/test_chemical_equilibrium.py -k compiler` and confirm failure because the private seam is absent.
- [ ] Implement dense rank/solve helpers and the immutable compiler with scale-aware residual checks.
- [ ] Bind only underscored native test functions and add the new sources to the existing native target.
- [ ] Re-run the focused compiler tests and retain only claim-bearing cases.
- [ ] Commit the design/compiler checkpoint after fresh focused and existing-suite verification.
...
- [ ] Write failing tests for neutral logs, multi-cation/multi-anion softmax shares, exact charge, round trip, species permutation, amount scale, and trace-floor classification.
- [ ] Add test-only centered directional checks for the Jacobian and all amount Hessians; confirm RED before implementation.
- [ ] Implement reference-category softmax value/first/second derivatives and exact `Q`/neutral exponential chain rules.
- [ ] Re-run chart tests at ordinary and trace scales, then the compiler tests.
- [ ] Commit the chart checkpoint after fresh verification.
...
- [ ] Write failing analytic solve tests for `A <-> B`, `2A <-> B`, and `A <-> C+ + D-`, with expected compositions derived independently in the tests.
- [ ] Write failing invariance tests for gauge, species order/permutation, amount scale, and reaction-basis transformations.
- [ ] Write failing negative tests for max-min infeasibility, trace-bound terminals, callback/domain failures, non-success Ipopt statuses, missing candidates, and synthetic false-success inputs.
- [ ] Implement the analytic ideal phase block and exact physical-to-chart objective chain rule.
- [ ] Implement the linear max-min TNLP and independently recompute strict-positive feasibility.
- [ ] Implement one homogeneous TNLP, direct `lambda=1` attempt, bounded continuation fallback, and mandatory final `lambda=1` acceptance.
- [ ] Implement independent postsolve recomputation, exact KKT assembly, and reduced-Hessian local-minimum adjudication.
- [ ] Run focused analytic and negative tests, then all chemical-equilibrium tests.
- [ ] Commit the native kernel checkpoint after fresh verification.
...
- [ ] Write a failing installed-artifact test using the immutable Perdomo Table-3 Provider wheel and synthetic reaction reference data.
- [ ] Add test-only directional gradient/Hessian checks around one strict interior Provider state.
- [ ] Implement the Provider phase adapter through `ProviderContext`, rejecting order/fingerprint/domain/packing mismatches before acceptance.
- [ ] Verify the installed case is labeled `manufactured_nonpredictive`, consumes all declared Provider callbacks, and makes no source-backed chemical claim.
- [ ] Commit the Provider-integration checkpoint after focused installed-artifact verification.
...
- [ ] Update package documentation to name the private foundation and preserve the accepted/public boundaries.
- [ ] Run focused pytest, full pytest, Ruff, mypy, native build warnings, and the installed-artifact case from the exact Provider wheel.
- [ ] Inspect `git diff`, staged diff, public exports, dependency edges, and generated artifacts.
- [ ] Run `bash "$HOME/.codex/hooks/codex-cleanup.sh" --repo-root .` and remove only task-owned ignored artifacts if reported.
